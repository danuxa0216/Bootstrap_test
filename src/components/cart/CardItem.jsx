...
const CardItem = (props) => {
  ...
  const onAddOverlay = async (obj)=>{
    try{
      ...
      if(findOverlay){
        axios.delete(`https://6441b7de76540ce2257d2335.mockapi.io/cart/${findOverlay.id}`)
        props.setOverlayItems((over) => over.filter(o=>o.myId !== obj.myId))
      }
      else{
        const {data} = await axios.post('https://6441b7de76540ce2257d2335.mockapi.io/cart', obj)
  ...
  const onAddFav = async (obj)=>{
    try{
      ...
      if(findFavorites){
        axios.delete(`https://6441b7de76540ce2257d2335.mockapi.io/cart/${findFavorites.id}`)
